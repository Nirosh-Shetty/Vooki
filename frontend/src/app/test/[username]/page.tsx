...
export default function CreatorPublicProfile() {
  ...
  const totalAudienceCount = useMemo(() => {
    if (!data) return 0;
    const ytCount = data.profile.stats?.youtube?.metrics?.subscribers || 0;
    const igCount = data.profile.stats?.instagram?.metrics?.subscribers || 0;
    const fbCount = data.profile.stats?.facebook?.metrics?.subscribers || 0;
    const sumSynced = ytCount + igCount + fbCount;
                ...
                <div className="flex flex-col sm:flex-row items-center sm:items-end gap-4 text-center sm:text-left">
                  <div className="relative shrink-0">
                    <img
                      ...
                      alt={data.name}
                      className="h-22 w-22 sm:h-24 sm:w-24 rounded-2xl object-cover border-4 border-[color:var(--vooki-app-surface)] shadow-md bg-[color:var(--vooki-app-surface)] ring-1 ring-[color:var(--vooki-app-border-strong)]"
                    />
                    {data.isVerified && (
                      <div
                        title="Verified Creator"
                        className="absolute -bottom-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-[color:var(--vooki-app-active-bg)] text-[color:var(--vooki-app-active-text)] border-2 border-[color:var(--vooki-app-surface)] shadow-xs"
                      >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 pt-5">
                <div className="rounded-2xl border border-[color:var(--vooki-app-border-strong)] bg-[color:var(--vooki-app-surface-strong)] p-3.5 flex flex-col justify-between">
                  ...
                  <p className="mt-2 text-xl font-extrabold text-[color:var(--vooki-app-text-strong)]">
                    {totalAudienceCount > 0 ? totalAudienceCount.toLocaleString() : "Verified"}
                  </p>
                </div>
